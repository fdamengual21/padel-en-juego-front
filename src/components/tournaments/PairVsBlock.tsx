    ...
    <div className={cn("flex w-full justify-start", className)}>
      <div className="inline-grid max-w-full grid-cols-[auto_auto_auto] items-center gap-x-2 text-left">
        <div className="min-w-0 max-w-[40vw] font-bold leading-tight sm:max-w-56">
          <p className="truncate capitalize">{a1}</p>
          {a2 ? <p className="truncate capitalize">{a2}</p> : null}
        </div>
        <span className="shrink-0 self-center px-1 text-center text-xs font-semibold uppercase tracking-wide text-sidebar">
          VS
        </span>
        <div className="min-w-0 max-w-[40vw] font-bold leading-tight sm:max-w-56">
          <p className="truncate capitalize">{b1}</p>
          {b2 ? <p className="truncate capitalize">{b2}</p> : null}
        </div>
      </div>
    </div>
  );
